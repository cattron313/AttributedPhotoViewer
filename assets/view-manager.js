(function (window) {
	...
	function ViewManager() {
		...
		return {
			...
			addNewImages(numImgs) {
				const container = document.getElementById('photos-container');
				for(let i = 0; i < numImgs; i++) { // assuming I will receive 20 images back
			...
			changePhotoViewerImg(direction){
				if (this.modalOpen()) {
					...
					const offset = ((direction < 0) ? photosList.length : 0);
					const thumbImgIndex = (shownImgIndex + direction + offset) % photosList.length;
					// making list circular
					const thumbImg = photosList[thumbImgIndex].firstChild;
					updatePhotoViewer.call(this, photoViewerImg, thumbImg, thumbImgIndex);
				}
			}
		};
...
window.aplb.view = new ViewManager();
})(window);
